...
- [x] 将正式页面注册调整为首页、读书会、书籍分区和我的，底部导航仅保留“首页 / 读书会 / 我的”；跑团、活动和助手源码保留但不再注册或提供用户入口。
- [x] 首页和读书会只展示赵玲玲老师的《成器比成功更重要—孔子的人生时教》《一生至少一次的哲学浴》，并接入公开书籍 API 同步。
- [x] 书籍分区提供作者、本书介绍、授权状态、音频主区、视频和 PDF 原书占位；内容为空时不出现旧课程报名表。
- [x] 新增全局 `BackgroundAudioManager` 服务，支持后台/锁屏元数据、播放暂停、拖动、前后 15 秒、倍速、15 秒心跳及暂停/结束进度上报。
- [x] “我的”改为已授权书籍入口，不再展示跑团数据和通用助手入口。
- [ ] `pnpm run typecheck` 已通过；仍需等待新书籍/授权/媒体 API 联调，并在微信开发者工具和真机验证后台播放、锁屏控制、视频及 PDF 打开能力。
